...
%define is_bclinux %{?bclinux:1}%{!?bclinux:0}
%define python38_exist %(test -f /usr/bin/python3.8; echo $?)
%if ((%{is_bclinux} && "%{bclinux}" == "8") || 0%{?alinux} == 3) && %{python38_exist} == 0
...
%endif
%else
%if 0%{?rhel} >= 8 || 0%{?fedora} >= 30 || (%{is_bclinux} && "%{bclinux}" == "22.10U2")
...
- Move from proprietary to extended.
- License verified
